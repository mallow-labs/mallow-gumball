...
/** Base error for the hand-written Gumball Machine / Gumball Guard client logic. */
export class GumballMachineError extends Error {
...
export class VariableSizeGuardError extends GumballMachineError {
  ...
  constructor(name: string) {
    super(
      `Trying add a guard [${name}] with a variable-size codec. ` +
        `The Gumball Guard program only works with fixed-size guards. ` +
        'Please use a fixed-size guard codec.'
    );
  }
}
  ...
  constructor(name: string) {
    super(
      `Trying to access a custom guard named [${name}] that ` +
        ...
        'Register your custom guard by adding it to the guard repository.'
    );
  }
}
...
export class GuardRouteNotSupportedError extends GumballMachineError {
  ...
  constructor(guardName: string) {
    super(
      `The provided guard [${guardName}] does not support the route instruction.`
    );
  }
}
