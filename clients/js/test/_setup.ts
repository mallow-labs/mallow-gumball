...
/**
 * Create the program-wide GlobalConfig PDA with `authority` as both the config
 * authority and the account-fee authority. Every ava test builds its own
 * LiteSVM ledger, so unlike the umi suite there is never an existing singleton
 * to update.
 */
