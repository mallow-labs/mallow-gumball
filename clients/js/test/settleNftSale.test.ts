...
test('it can settle an nft sale after nft has been claimed and burnt', async (t) => {
  ...
  // The buyer claims the nft and burns it in the same transaction, so its
  // token account, metadata and master edition are all gone before settle.
  ...
  await sendTransaction(client.svm, buyer, [
    ...
    await getBurnNftInstruction({ owner: buyer, mint }),
  ]);
  t.false(client.svm.getAccount(buyerAta).exists);
  ...
  // Settling must still pay the seller although the nft no longer exists.
  await sendTransaction(client.svm, client.payer, [
    COMPUTE_UNITS,
    await getSettleNftSaleInstructionAsync({
      index: 0,
