...
// token2022Payment now has its own jellybean branch, mirroring tokenPayment:
// the price is pulled from the payer's Token-2022 account into the fee
// accounts' Token-2022 ATAs. `draw_jellybean` keeps its named `spl_token_program`
// on classic SPL, so the Token-2022 program rides in as a remaining account
// (TOKEN22_PLAN §D5) — the layout is `[payer ATA, mint, T22 program, ...fee ATAs]`.
test('it distributes a token2022Payment to the jellybean fee account on draw', async (t) => {
  const umi = await createJellybeanUmi(createUmi);
  ...
  const [tokenMint, payerAta, feeAta] = await createMintWithHolders(
  ...
  const jellybeanMachine = await setupJellybeanMachine(umi, {
    ...
    guards: {
      token2022Payment: some({
        mint: tokenMint.publicKey,
        amount: 5,
        // Jellybean pays the machine's fee accounts, so the stored destination
        // is unused; the guard reads the fee ATAs from the remaining accounts.
        destinationAta: feeAta,
      }),
    },
  });
  ...
  // The fee account's Token-2022 account received the 5-token price...
  t.is((await fetchToken(umi, feeAta)).amount, 5n);
  // ...and the payer paid it.
  t.is((await fetchToken(umi, payerAta)).amount, 995n);
});

// The guard mint is the sole source of truth on jellybean (no machine
// payment_mint to cross-check), so a draw naming a different mint than the
// stored guard must fail rather than silently charge the wrong token.
test('it rejects a token2022Payment jellybean draw naming a different mint', async (t) => {
  const umi = await createJellybeanUmi(createUmi);
  const programsWithToken22 = umi.programs.clone();
  programsWithToken22.bind('splToken', 'splToken2022');
  const feeWallet = generateSigner(umi).publicKey;
  ...
  const [tokenMint, , feeAta] = await createMintWithHolders(
    { ...umi, programs: programsWithToken22 },
    {
      holders: [
        { owner: payer.publicKey, amount: 1000 },
  ...
  const [otherMint] = await createMintWithHolders(
    { ...umi, programs: programsWithToken22 },
    { holders: [{ owner: payer.publicKey, amount: 1000 }] }
  );
