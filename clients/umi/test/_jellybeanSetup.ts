...
// `mallowJellybean()` replaces `umi.guards` with a repository built from the
// *published* `@mallow-labs/mallow-gumball` manifests, so re-applying
// `mallowGumball()` after it is what makes these tests exercise the guard
// clients in this workspace rather than the ones on npm.
export const createJellybeanUmi = async (
  createUmi: () => Promise<Umi>
): Promise<Umi> =>
  (await createUmi()).use(mallowJellybean()).use(mallowGumball());
