    ...
    fn verify(proof: &[[u8; 32]], root: &[u8; 32], leaf: &[u8; 32]) -> bool {
        ...
        for proof_element in proof.iter() {
            if computed_hash <= *proof_element {
                ...
                computed_hash = solana_program::keccak::hashv(&[&computed_hash, proof_element]).to_bytes()
            } else {
                // hash (current element of the proof + current computed hash)
                computed_hash = solana_program::keccak::hashv(&[proof_element, &computed_hash]).to_bytes();
            }
        }
